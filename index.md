# Minhyeok Kang

📍 Seoul, South Korea  
📧 [alsgur434@gmail.com](mailto:alsgur434@gmail.com)  
🔗 [Homepage](https://mmlab.snu.ac.kr/~mhkang) | [LinkedIn](https://www.linkedin.com/in/minhyeok-kang/) | [GitHub](https://github.com/KangMinHyeok)

---

## 🧠 Professional Summary

Accomplished network security professional with 7+ years of academic expertise in Internet security, specializing in comprehensive infrastructure protection. Proven track record of developing and implementing practical security solutions through rigorous data-driven analysis. Advanced expertise in security domains including RPKI, BGP, PKI, TLS, with a strategic approach to identifying and mitigating network threats.

---

## 🎓 Education

**Texas A&M University, USA**  
*Visiting Researcher in Computer Science and Engineering*  
Dec 2024 – Jun 2025

**Seoul National University, Korea**  
*Integrated M.S./Ph.D. in Computer Science and Engineering — Focus: Internet Security Protocols*  
Sep 2017 – Aug 2025

**Seoul National University, Korea**  
*B.S. in Computer Science and Engineering*  
Mar 2012 – Jul 2017

---

## 🔬 Research and Projects

### IRRedicator – NDSS 2024  
Conducted a comprehensive 12-year longitudinal study on BGP traces using Spark and HDFS to investigate routing security inconsistencies between RPKI and IRR. Filtered 58% of stale IRR route objects using an ensemble model, achieving 93% BGP validation accuracy.

### FASTEN – IoT Security Framework (PL)  
Developed a framework enabling secure communication between IoT devices and applications with 0-RTT data transmission, enhancing data protection and performance.

### Next-generation Cloud-native Cellular Network Leadership Program - 5G/O-RAN Security  
Analyzed 5G/O-RAN architecture vulnerabilities through strategic attack/defense design. Demonstrated a signaling storm attack targeting the control unit to reveal security weaknesses.

---

## 📚 Publications

- **IRRedicator: Pruning IRR with RPKI-Valid BGP Insights**  
  *Minhyeock Kang, Weitong Li, Roland van Rijswijk-Deij, Ted "Taekyoung" Kwon, and Taejoong Chung*, NDSS 2024

- **How to decentralize the internet: A focus on data consolidation and user privacy**  
  *Ted “Taekyoung” Kwon et al., including Minhyeok Kang*, Computer Networks 2023

- **TwinPeaks: An Approach for Certificateless Public Key Distribution for the Internet and IoT**  
  *Eunsang Cho et al., including Minhyeok Kang*, Computer Networks 2020

- **An SGX-Based Key Management Framework for Data Centric Networking**  
  *Minkyung Park et al., including Minhyeok Kang*, IEEE Access 2020

---

## 🛠️ Skills and Tools

**Strong:** BGP, RPKI, IRR, TLS, DTLS, Spark, HDFS, Python  
**Knowledgeable:** Classification with rejection, Explainable AI (Sharp), OSCORE, ACE Framework, 5G/O-RAN security, Cryptography, C/C++, Linux, Flutter, Nest.js, PostgreSQL, Git

---
